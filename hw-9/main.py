...
def sha256(s: str):
    ...
    """
    return "".join(f"{b:02x}" for b in hashlib.sha256(s.encode()).digest())
...
def authenticate(username, password):
    ...
    False
    """
    return sha256(password) == users[username]

chars = 'abcdefghijklmnopqrstuvxyz'

def hack_sha256_fixed_size(sha256_hash, n):
    """
    ...
    'elte'
    """
    return next(("".join(comb) for comb in itertools.product(*(chars,)*n) if sha256("".join(comb)) == sha256_hash), None)

def hack_sha256(sha256_hash):
    """
    ...
    'elte'
    """
    return next((pw for i in range(1, 11) if (pw := hack_sha256_fixed_size(sha256_hash, i))), None)

# I used https://crackstation.net/
# As per the website: CrackStation uses massive pre-computed lookup tables to crack password hashes.
...
def authenticate_with_pepper(username, password):
    ...
    False
    """
    return sha256(pepper_prefix + password) == users_with_pepper[username]["passwordHash"]
...
def authenticate_with_pepper_and_salt(username, password):
    ...
    False
    """
    return sha256(pepper_prefix + password + users_with_pepper_and_salt[username]["salt"]) == users_with_pepper_and_salt[username]["passwordHash"]